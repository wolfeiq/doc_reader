...
export function FilterBtn({ active, onClick, label, variant }: { active: boolean, onClick: () => void, label: string, variant: FilterVariant }) {
  ...
  return <button onClick={onClick} className={cn(base, styles[variant])}>{label}</button>;
}